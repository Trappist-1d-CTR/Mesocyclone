...
I'm currently developing it with [**half bread**](https://github.com/halfbread-BP) (started solo), system by system, through the months. We've made a demo you can find on Itch.io over **https://trappist-1-d.itch.io/mesocyclone!!!** :D
...
## Development
You can take a look at the game's development through my [**TigForums topic**](https://forums.tigsource.com/index.php?topic=76539.0)
...
You can find the ones we coded, though, by peeking into the Assets Folder and locating all the ones marked with the following [Dev#] (# being a number) and then opening the folder named "Script(s)" (only applies for the earlier ones however).
...
- **[Dev5]** is the environment: the terrain. Not much more to it, but it's useful to get some points of reference of the drone's speed and controls. This is soon to be deprecated as we replace it with the ECS Voxel Terrain System.
...
- **[Dev9]** Is the audio engine, which, well, serves as a framework to play audio clips. However now we use [**FMOD**](https://www.fmod.com/) and [**Steam Audio**](https://valvesoftware.github.io/steam-audio/) to have more mixing control over the audio.
- **[Dev10]** This is specifically for the game's custom tick system, so we can support dynamic tick rates for performance. This is also soon to be deprecated as we rebuild the game in DOTS.
- **[Dev11]** is the Climate Model Simulation [Astraa i give u the honors to fill this out plz]
...
- IDE / Code editor ([Visual Studio](https://visualstudio.microsoft.com/vs/community/), [VSCode](https://code.visualstudio.com/), [Rider](https://www.jetbrains.com/rider/download/), [NeoVim](https://neovim.io/), etc.)
...
- Load the project: Open Unity Hub, Click on **"Add"** and select **"Add from disk"**, Click on the project when it appears.
...
# Copyright
In short: Look up [**MIT Copyright License**](https://en.wikipedia.org/wiki/MIT_License) BUT you can't get the entirety of it and use it commercially, only snippets.
