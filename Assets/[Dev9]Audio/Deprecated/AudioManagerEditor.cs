...
//DO YOU HAVE ANY IDEA HOW MUCH YOU HAVE DOOMED ME TO DESPAIR WITH THIS ONE CHOICE??!??!?!?!?
...
#nullable enable // in case this isn't disabled
    ...
    public class AudioManagerPlayWindow : EditorWindow
    {
        ...
        float minPitch = 0.9f;
        float maxPitch = 1.1f;
        Vector3 windowPosition = default; // Vector3.zero
        bool is2D = true;
        float volume = 1f;
        ...
        {
            AudioManagerPlayWindow window = GetWindow<AudioManagerPlayWindow>();
        ...
        void OnGUI()
        {
            ...
            windowPosition = EditorGUILayout.Vector3Field("Position", windowPosition);
            ...
            if (GUILayout.Button("Play"))
            {
                if (!isRepeating)
                {
                    if (clip != null)
                    {
                        ...
                        (
                            clip, minPitch, maxPitch, windowPosition, is2D, volume, minDistance, maxDistance, rolloffMode
                        );
                    }
                }
