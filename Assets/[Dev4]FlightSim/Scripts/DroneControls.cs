...
public sealed partial class DroneControls : MonoBehaviour
{
    ...
    #region Drone Data
    [System.Serializable]
    private sealed class MainStats
    {
        private MainStats()
        { }
...
for (int i = 0; i < NetLinker.Parts.DronePartStats.Length; i++)
{
    ...
    //
    #region Torque
    //PhysicsAngAcceleration += Check = 0.5f * C.GaleAtmD * TorqueAoA[i].Evaluate(AoA) * NetLinker.Parts.DronePartStats[i].Area * AirSpeed.sqrMagnitude * Memory / DronePhysics.mass;
    if (Time.time > 10)
    {
    //    Debug.Log("i = " + i + " : " + Check + " ; " + Memory);
    }
...
/*
DynamicPressure = C.GaleAtmD * RefAirSpeed1;
...
if (DynamicPressure >= threshold1) //Above 18 m/s
{
    ...
    {
        pctrl = CanardsCtrl * (50 * C.GaleAtmD / DynamicPressure);
        rctrl = AileronsCtrl * (50 * C.GaleAtmD / DynamicPressure);
        yctrl = ElevatorsCtrl * (50 * C.GaleAtmD / DynamicPressure);
        //yctrl = YawCtrl * ((C.GaleAtmD / C.GaleAtmD) * (900 / RefSqrAirSpeed7));
