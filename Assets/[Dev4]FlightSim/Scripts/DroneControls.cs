...
namespace Mesocyclone
{
    ...
    public sealed partial class DroneControls : Tickable
    {
        ...
        [System.Serializable]
        public sealed class MainDroneStats
        {
            private MainDroneStats()
            { }
        ...
        [System.Serializable]
        public sealed class DronePartsList
        {
            private DronePartsList()
            { }
        ...
        [System.Serializable]
        public sealed class DroneObject
        {
            /*private DroneObject()
            { }*/
        ...
        public enum HoverModeType : int
        {
            ...
            // re- half bread: are we high?? 5 is right there¿
            // someone check the commit history ;-;
        }
        ...
        public override void FixedTick()
        {
            ...
            if (FLIPPerforming)
            {
                ...
                if (VisualizationMode == VisualizationModeType.Torque)
                    UnityEngine.Debug.DrawLine(transform.position + FLIPThrustPos, transform.position + FLIPThrustPos + (DronePhysics.mass * Memory / 100), Color.blue, 1 / Time.renderedFrameCount);
            ...
            for (int i = 0; i < NetLinker.Parts.DronePartStats.Length; i++)
            {
                ...
                #region Lift
                Memory = t.up;
                PhysicsAcceleration += Check = 0.5f * GlobalCalcGO.DensityAtHeight(PhysicsPosition.y) * LiftAoA[i].Evaluate(AoA) * NetLinker.Parts.DronePartStats[i].Area * AirSpeed.sqrMagnitude * Memory / DronePhysics.mass;
                TotLift += Check;
                ...
                if (VisualizationMode == VisualizationModeType.LiftDrag)
                    UnityEngine.Debug.DrawLine(t.position, t.position + (DronePhysics.mass * Check / 20), Color.green, 1 / Time.renderedFrameCount);
                #endregion

                #region Induced Drag
                Memory = -AirSpeed.normalized;
                PhysicsAcceleration += Check = 0.5f * GlobalCalcGO.DensityAtHeight(PhysicsPosition.y) * InducedDragAoA[i].Evaluate(AoA) * NetLinker.Parts.DronePartStats[i].Area * AirSpeed.sqrMagnitude * Memory / DronePhysics.mass;
                TotDrag += Check;

                if (VisualizationMode == VisualizationModeType.LiftDrag)
                    UnityEngine.Debug.DrawLine(t.position, t.position + (DronePhysics.mass * Check / 20), Color.red, 1 / Time.renderedFrameCount);
                #endregion

                #region Torque

                Memory = 0.5f * GlobalCalcGO.DensityAtHeight(PhysicsPosition.y) * TorqueAoA[i].Evaluate(AoA) * NetLinker.Parts.DronePartStats[i].Area * AirSpeed.sqrMagnitude * ((i != 7) ? Vector3.up : Vector3.forward);
                PhysicsTorque += Check = Vector3.Cross(L - DronePhysics.centerOfMass, Memory);

                //if (i == 0) Debug.Log("i=" + i + " ; AoA " + AoA + " => " + Check.z);

                if (VisualizationMode == VisualizationModeType.Torque)
                    UnityEngine.Debug.DrawLine(t.position, t.position + (DronePhysics.mass * Memory / 100), Color.blue, 1 / Time.renderedFrameCount);
                #endregion
                ...
                if (NetLinker.Parts.DronePartStats[i].IDb != 0)
                {
                    ...
                    if (VisualizationMode == VisualizationModeType.LiftDrag)
                        UnityEngine.Debug.DrawLine(CM + (DronePhysics.rotation * L), CM + (DronePhysics.rotation * L) + (DronePhysics.mass * Check / 20), Color.green, 1 / Time.renderedFrameCount);
                    #endregion
            ...
            DynamicPressure = GlobalCalcGO.DensityAtHeight(PhysicsPosition.y) * AirSpeed.sqrMagnitude;
            ...
            */
            ...
            //Forward Drag
            PhysicsAcceleration += Check = 0.5f * GlobalCalcGO.DensityAtHeight(PhysicsPosition.y) * (InputControl.FlightControls.AirBrakes.IsPressed() ? s.AirBrakesCd : s.FrontCd) * s.FrontArea * Mathf.Pow(Vector3.Dot(AirSpeed, Memory), 2) * Mathf.Sign(Vector3.Dot(AirSpeed, Memory)) * Memory / DronePhysics.mass;
            TotDrag += Check;
            ...
            //Vertical Drag
            PhysicsAcceleration += Check = 0.5f * GlobalCalcGO.DensityAtHeight(PhysicsPosition.y) * s.BottomCd * s.BottomArea * Mathf.Pow(Vector3.Dot(AirSpeed, Memory), 2) * Mathf.Sign(Vector3.Dot(AirSpeed, Memory)) * Memory / DronePhysics.mass;
            TotDrag += Check;
        ...
        public override void Tick()
        {
            ...
            switch (VisualizationMode)
            {
                ...
                case VisualizationModeType.Forces:
                    UnityEngine.Debug.DrawLine(CM, CM + (PhysicsVelocity / 20), Color.yellow, 1 / Time.renderedFrameCount);

                    UnityEngine.Debug.DrawLine(CM, CM + (DronePhysics.mass * TotThrust / 20), Color.blue, 1 / Time.renderedFrameCount);
                    UnityEngine.Debug.DrawLine(CM, CM + (DronePhysics.mass * TotDrag / 20), Color.red, 1 / Time.renderedFrameCount);
                    UnityEngine.Debug.DrawLine(CM, CM + (DronePhysics.mass * TotLift / 20), Color.green, 1 / Time.renderedFrameCount);
                    UnityEngine.Debug.DrawLine(CM, CM + (DronePhysics.mass * TotWeight / 20), Color.grey, 1 / Time.renderedFrameCount);
                    break;
                ...
                case VisualizationModeType.Torque:
                    ...
                    UnityEngine.Debug.DrawLine(CM - (DronePhysics.rotation * new Vector3(0, 0.2f)), CM + (DronePhysics.rotation * new Vector3(0, 0.2f)), Color.yellow, 1 / Time.renderedFrameCount);
                    UnityEngine.Debug.DrawLine(CM - (DronePhysics.rotation * new Vector3(0, 0, 0.2f)), CM + (DronePhysics.rotation * new Vector3(0, 0, 0.2f)), Color.yellow, 1 / Time.renderedFrameCount);
        ...
        public void CalculateFlightCoefficients(int i)
        {
            ...
            for (int i2 = 0; i2 < d.StallAngle / iStep; i2++)
            {
                ...
                CT = d.FrontCd * Mathf.Cos(AoA);
                CN = (((i2 * iStep) * memmoi) + (CT * Mathf.Sin(AoA))) / Mathf.Cos(AoA);
                CM = CN * (0.25f - (0.175f * (1 - (2 * AoA / Mathf.PI))));
            ...
            CT = d.FrontCd * Mathf.Cos(AoA);
            CN = ((d.StallAngle * memmoi) + (CT * Mathf.Sin(AoA))) / Mathf.Cos(AoA);
            CM = 5 * CN * (0.25f - (0.175f * (1 - (2 * AoA / Mathf.PI))));
